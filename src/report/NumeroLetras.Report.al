report 50561 "Check Translation Management"
{
    ApplicationArea = All;
    DefaultLayout = RDLC;
    UsageCategory = ReportsAndAnalysis;
    RDLCLayout = 'report\rdl\CheckTranslationManagement.rdl';
    ...
    procedure FormatNoText(var NoText: array[2] of Text[80]; No: Decimal; NewLanguageCode: Integer; NewCurrencyCode: Code[10]; Moneda: Text) Result: Boolean
    begin
        ...
        if CurrencyCode = '' then begin
            ...
            case GLSetup."LCY Code" of
                'USD':
                    Currency.Description := 'DOLARES';
                'PEN':
                    Currency.Description := 'SOLES';
                'CAD':
        ...
        if No < 1000000000000.0 then
            case LanguageCode of
                ...
                SpanishLanguageCode:
                    Result := FormatNoTextESM(NoText, No, Moneda);
                FrenchLanguageCode:
    ...
    begin
        ...
        case LanguageCode of
            ...
            SpanishLanguageCode:
                if NoText[NoTextIndex] = Text104 then BEGIN
                    IF (AddText = 'Cien')
                    THEN begin
                        AddText := 'CIENTO';
                    end;
                    IF (AddText = 'SOLES') OR (AddText = 'Dolares')
                    THEN begin
                        AddText := '';
                    end;
                    NoText[NoTextIndex] := DelChr(NoText[NoTextIndex] + UpperCase(AddText), '<')
    ...
    local procedure FormatNoTextESM(var NoText: array[2] of Text[80]; No: Decimal; Moneda: Text): Boolean
    var
        PrintExponent: Boolean;
        Ones: Integer;
        Tens: Integer;
    ...
    begin
        ...
        if (NroD = '0') then begin
            NroD := '00';
        end;
        exit(AddToNoText(NoText, NoTextIndex, PrintExponent, 'CON ' + NroD + CentsText + ' ' + Moneda, ' '));
    end;
